...
import { Tv, Play, Clock, ListVideo, Star, Calendar } from "lucide-react"
...
const INITIAL_WIDGETS: TVWidget[] = [
  { id: "currently-watching", size: { width: 3, height: 3 }, position: { x: 0, y: 0 }, visible: true },
  { id: "episodes-today", size: { width: 2, height: 2 }, position: { x: 3, y: 0 }, visible: true },
  { id: "watch-time", size: { width: 2, height: 2 }, position: { x: 5, y: 0 }, visible: true },
  { id: "watchlist", size: { width: 3, height: 3 }, position: { x: 7, y: 0 }, visible: true },
  { id: "recently-finished", size: { width: 4, height: 3 }, position: { x: 0, y: 3 }, visible: true },
  { id: "top-rated", size: { width: 3, height: 2 }, position: { x: 4, y: 3 }, visible: true },
]
...
export default function TVPage() {
  const [widgets, setWidgets] = useState<TVWidget[]>(INITIAL_WIDGETS)
  ...
  const findOverlappingWidgets = useCallback(
    (x: number, y: number, width: number, height: number, excludeId: string): TVWidget[] => {
      const overlapping: TVWidget[] = []
      for (const widget of visibleWidgets) {
        if (widget.id === excludeId) continue
        const overlaps =
          x < widget.position.x + widget.size.width &&
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    const gridPos = pixelToGrid(originalLeft + delta.x, originalTop + delta.y)
  ...
  const renderWidget = (widget: TVWidget, isOverlay = false) => {
        ...
        return (
          <Card className={cn(baseClasses, "bg-card border-border")}>
            ...
            <CardContent className="space-y-2">
              <div className="text-lg font-semibold">Breaking Bad</div>
              <p className="text-xs text-muted-foreground">Season 4, Episode 12</p>
          ...
          <Card className={cn(baseClasses, "bg-card border-border")}>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                {["The Bear", "Severance", "Shogun", "The Last of Us"].map((show) => (
        ...
        return (
          <Card className={cn(baseClasses, "bg-card border-border")}>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                {[
                  { name: "True Detective S1", date: "Dec 10" },
                  { name: "The Office", date: "Dec 5" },
                  { name: "Dark", date: "Nov 28" },
                ].map((show) => (
                  <div key={show.name} className="flex justify-between">
                    <span>{show.name}</span>
          ...
          <Card className={cn(baseClasses, "bg-card border-border")}>
            ...
            <CardContent>
              <div className="space-y-1 text-sm">
                {["Breaking Bad", "The Wire", "Chernobyl"].map((show, i) => (
                  <div key={show} className="flex items-center gap-2">
