...
describe('TrackerDetailView social CRM surface', () => {
  ...
  beforeEach(() => {
    ...
    mocks.getTrackerIdentityMock.mockReturnValue('social')
  ...
  it('exposes a manage contacts CTA and renders structured social contacts', async () => {
    render(<TrackerDetailView trackerId={tracker.id} selectedDate={selectedDate} assets={new Map()} />)

    expect(screen.getByText('Jack Robert')).toBeTruthy()
    expect(screen.getByText('text')).toBeTruthy()
    expect(screen.getByText('text check-in')).toBeTruthy()
    expect(screen.getByText(/positive/i)).toBeTruthy()

    fireEvent.click(screen.getByRole('button', { name: /manage contacts/i }))

    expect(mocks.setSelectedContactIdMock).toHaveBeenCalledWith(undefined)
    expect(mocks.setCurrentPageMock).toHaveBeenCalledWith('contacts')
  })
})
