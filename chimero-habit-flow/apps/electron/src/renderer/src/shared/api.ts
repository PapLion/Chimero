...
const webApi: ElectronApi = {
  ...
  addFoodEntry: (data) => request('/api/food/entries', { method: 'POST', body: data }),
  updateFoodEntry: (entryId, updates) => request(`/api/food/entries/${entryId}`, { method: 'PUT', body: updates }),
  deleteFoodEntry: (entryId) => request(`/api/food/entries/${entryId}`, { method: 'DELETE' }),
  getFoodDetail: (trackerId, options) => request(`/api/food/trackers/${trackerId}/detail${queryString(options)}`),
  addIntakeEntry: (data) => request('/api/intake/entries', { method: 'POST', body: data }),
  updateIntakeEntry: (entryId, updates) => request(`/api/intake/entries/${entryId}`, { method: 'PUT', body: updates }),
  deleteIntakeEntry: (entryId) => request(`/api/intake/entries/${entryId}`, { method: 'DELETE' }),
  getIntakeDetail: (trackerId, options) => request(`/api/intake/trackers/${trackerId}/detail${queryString(options)}`),
  getIntakeHomeWidget: (trackerId, options) => request(`/api/intake/trackers/${trackerId}/home${queryString(options)}`),
  addHealthSymptomEntry: (data) => request('/api/health/entries', { method: 'POST', body: data }),
  updateHealthSymptomEntry: (entryId, updates) => request(`/api/health/entries/${entryId}`, { method: 'PUT', body: updates }),
  deleteHealthSymptomEntry: (entryId) => request(`/api/health/entries/${entryId}`, { method: 'DELETE' }),
  getHealthDetail: (trackerId, options) => request(`/api/health/trackers/${trackerId}/detail${queryString(options)}`),
  getHealthHomeWidget: (trackerId, options) => request(`/api/health/trackers/${trackerId}/home${queryString(options)}`),
  ...
  getWorkoutSession: (entryId) => request(`/api/workouts/sessions/${entryId}`),
  getWorkoutHistory: (trackerId, options) => request(`/api/workouts/trackers/${trackerId}/history${queryString(options)}`),
  createWorkoutSession: (data) => request('/api/workouts/sessions', { method: 'POST', body: data }),
  updateWorkoutSession: (entryId, updates) => request(`/api/workouts/sessions/${entryId}`, { method: 'PUT', body: updates }),
  deleteWorkoutSession: (entryId) => request(`/api/workouts/sessions/${entryId}`, { method: 'DELETE' }),
  getWorkoutRoutines: (trackerId) => request(`/api/workouts/trackers/${trackerId}/routines`),
  getWorkoutRoutine: (routineId) => request(`/api/workouts/routines/${routineId}`),
  createWorkoutRoutine: (data) => request('/api/workouts/routines', { method: 'POST', body: data }),
  updateWorkoutRoutine: (routineId, updates) => request(`/api/workouts/routines/${routineId}`, { method: 'PUT', body: updates }),
  deleteWorkoutRoutine: (routineId) => request(`/api/workouts/routines/${routineId}`, { method: 'DELETE' }),
  instantiateWorkoutFromRoutine: (data) => request('/api/workouts/routines/instantiate', { method: 'POST', body: data }),
  saveWorkoutAsRoutine: (data) => request(`/api/workouts/sessions/${data.sessionEntryId}/save-as-routine`, { method: 'POST', body: data }),
  getWorkoutHome: (trackerId) => request(`/api/workouts/trackers/${trackerId}/home`),
  getWorkoutStatistics: (trackerId) => request(`/api/workouts/trackers/${trackerId}/stats`),
  getWorkoutGraph: (trackerId) => request(`/api/workouts/trackers/${trackerId}/graph`),
  getWorkoutCalendar: (trackerId, year, month) => request(`/api/workouts/trackers/${trackerId}/calendar/${year}/${month}`),
  getExerciseProgress: (trackerId, exerciseId) => request(`/api/workouts/trackers/${trackerId}/progress/${encodeURIComponent(exerciseId)}`),
  getContacts: (options) => request(`/api/contacts${queryString(options)}`),
  ...
  getContactReminderSettings: (contactId) => request(`/api/contacts/${contactId}/reminder-settings`),
  upsertContactReminderSettings: (data) => request(`/api/contacts/${data.contactId}/reminder-settings`, { method: 'PUT', body: data }),
  getContactProfileBlocks: (contactId) => request(`/api/contacts/${contactId}/profile-blocks`),
  createContactProfileBlock: (data) => request(`/api/contacts/${data.contactId}/profile-blocks`, { method: 'POST', body: data }),
  updateContactProfileBlock: (id, updates) => request(`/api/contact-profile-blocks/${id}`, { method: 'PUT', body: updates }),
  deleteContactProfileBlock: (id) => request(`/api/contact-profile-blocks/${id}`, { method: 'DELETE' }),
  reorderContactProfileBlocks: (contactId, ids) => request(`/api/contacts/${contactId}/profile-blocks/reorder`, { method: 'PUT', body: { ids } }),
