...
| Exercises | Search/select exercise records for logging context and structured workout session logging. | `ExerciseSearch.tsx`, `ExerciseDownloadToast.tsx`, Quick Entry exercise branch, tracker detail exercise branch | `features/exercises/service.ts`, `features/exercises/handler.ts` | `features/exercises`, workout session metadata helpers | exercise cache file, generic `entries.metadata` | PARTIAL | `Exercise.md` | Exercise search exists; structured workout session metadata is now visible in the UI, but dedicated workout tables, load/PR analytics, and deeper routine management remain future. |
...
- Exercise workout logging now carries structured session/routine/set metadata on the generic entry backbone; dedicated workout tables and deeper analytics remain future.
