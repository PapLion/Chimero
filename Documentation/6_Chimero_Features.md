...
| Exercises | Structured workout logging plus exercise search/database support. | `ExerciseSearch.tsx`, `ExerciseDownloadToast.tsx`, Quick Entry workout builder, tracker detail exercise branch, home widget, calendar summary | `features/exercises/service.ts`, `features/exercises/handler.ts`, workout service/handler, calendar/tracking services | `features/exercises`, workout session/routine read models | exercise cache file, `entries`, `workout_sessions`, `workout_session_exercises`, `workout_sets`, `workout_routines`, `workout_routine_exercises` | ACTIVE | `Exercise.md` | Legacy generic Exercise rows remain readable as unstructured history; cardio/steps, estimated 1RM, and mixed-unit analytics remain out of scope. |
...
- Exercise workout logging is DB-backed with dedicated workout tables, while legacy generic rows remain readable as unstructured history.
