...
| generic entry | Tracker data stored only in `entries.value`, `entries.note`, and `entries.metadata`. | `entry/handler.ts`, non-Weight tracker docs | Log Savings as numeric entry. | You assume specialized fields exist when they do not. | Maybe. |
